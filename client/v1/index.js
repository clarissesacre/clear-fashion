...
{
  list_of_brands.push(marketplace[i]['brand']);
}
...
// we sort : lower to highest price
console.log("todo 11");
function sort_by_price3(brand)
{
  return brand.sort((a, b) => {
...
for(let i in brands)
{
  brands[i]=sort_by_price3(brands[i]);
}
...
for(let i in brands)
{
  ...
  console.log(i , brands[i][index]['price']);
}
...
console.log(COTELE_PARIS[1]['released']);
for(let i = 0 ; i<=COTELE_PARIS.length ; i++)
{
  if(new Date(COTELE_PARIS[i]['released']) >= twoWeeksAgo)
  {
    const newprod = False;
